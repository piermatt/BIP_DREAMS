...
bool rtcOK=false,sdOK=false,ahtOK=false,bmpOK=false,spsOK=false;
...
void setup(){
  ...
  sps.stopMeasurement();
  int16_t err=sps.startMeasurement(SPS30_OUTPUT_FORMAT_OUTPUT_FORMAT_UINT16);
  if(err==NO_ERROR){
    spsOK=true;Serial.println(F("SPS30 OK"));
    delay(8000);  // fan + laser warm-up
  } else Serial.println(F("SPS30 missing — check SEL→GND"));

  Serial.println(F("timestamp,temp_C,rh_pct,pres_hPa,PM1,PM25,PM4,PM10,NC05,NC1,NC25"));
...
void loop(){
  ...
  if(now-lastSample>=SAMPLE_MS){
    ...
    // SPS30 — uint16 values are already scaled ×10 (0.1 µg/m³, 0.1 #/cm³)
    long pm1=-1,pm25=-1,pm4=-1,pm10=-1,nc05=-1,nc1=-1,nc25=-1;
    if(spsOK){
      uint16_t rdy=0;
      sps.readDataReadyFlag(rdy);
      if(rdy){
        ...
        if(sps.readMeasurementValuesUint16(m1,m25,m4,m10,n05,n1,n25,n4,n10,ts)==NO_ERROR){
          ...
          nc05=(long)n05; nc1=(long)n1; nc25=(long)n25;
        }
      }
    }
    ...
    row[pos++]=',';pos=appendFP1(row,pos,nc05);
    row[pos++]=',';pos=appendFP1(row,pos,nc1);
    row[pos++]=',';pos=appendFP1(row,pos,nc25);
    row[pos]='\0';
    Serial.println(row);
    ...
    if(sdOK){
      ...
      if(f){if(isNew)f.println(F("timestamp,temp_C,rh_pct,pres_hPa,PM1,PM25,PM4,PM10,NC05,NC1,NC25"));
